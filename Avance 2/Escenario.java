...
public class Escenario {
    ...
    public void crear_pistas(){
        ...
        for(int i=0; i<cantidad_pistas; i++){
            ...
            getPistas().get(i).setPosición_x(x);
            //Ingresar posición y.
            ...
            getPistas().get(i).setOrientación(respuesta);
            ...
            while(getPistas().get(i).getOrientación() == 1 && (largo_pista + getPistas().get(i).getPosición_x() > getEje_x() || largo_pista%50 != 0)){
                System.out.println("Intente nuevamente");
                largo_pista = lector.nextInt();
            }
            while(getPistas().get(i).getOrientación() == 2 && (largo_pista + getPistas().get(i).getPosicion_y() > getEje_y() || largo_pista%50 != 0)){
                System.out.println("Intente nuevamente");
                largo_pista = lector.nextInt();
            }
        ...
        for(int i = 0; i < getEje_y(); i = i+50){
            for(int j = 0; j < getEje_x(); j = j+50){
                for(int k = 0; k < getPistas().size(); k++){
                    if(getPistas().get(k).getOrientación() == 1){
                        if(j >= getPistas().get(k).getPosición_x() && j < getPistas().get(k).getPosición_x()+getPistas().get(k).getLargo_pista() && getPistas().get(k).getPosicion_y() == i ){
                            pista_encontrada = true;
                        }
                    }
                    if(getPistas().get(k).getOrientación() == 2){
                        if(i >= getPistas().get(k).getPosicion_y() && i < getPistas().get(k).getPosicion_y() + getPistas().get(k).getLargo_pista() && getPistas().get(k).getPosición_x() == j){
                            pista_encontrada = true;
                        }
                    }
                }
